...
./gradlew bootRun
```
필요 환경변수: `DB_URL`, `DB_USERNAME`, `DB_PASSWORD`, `AWS_ACCESS_KEY`, `AWS_SECRET_KEY`, `AWS_BUCKET_NAME`, `AWS_REGION_NAME`, `KAKAO_CLIENT_ID`, `KAKAO_CLIENT_SECRET`, `KAKAO_REDIRECT_URI`, `MAIL_USERNAME`, `MAIL_PASSWORD`, `WEATHER_API_KEY`, `PORT_ONE_API_KEY`, `PORT_ONE_SECRET_KEY`, `PUBLIC_KEY`, `PRIVATE_KEY`, `WEBPUSH_SUBJECT`, `VUE_APP_ENCRYPTION_KEY` 등
