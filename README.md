...
- **프로젝트 문서화**: 프로젝트 개요, 아키텍처, 기술 선택 이유, 실행 방법 등을 항목별로 정리하고 다이어그램·배지를 추가해 가독성을 높였습니다. 또한 주요 시나리오(주문 생성 → 장바구니 → 결제)를 흐름 순서대로 재편집하고 역할별(농부·구매자) 시나리오로 분리하여 시연 영상의 전달력을 개선했습니다.
